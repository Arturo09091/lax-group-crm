...
if (process.env.DATABASE_URL) {
    ...
    // Railway internal Postgres does NOT use SSL — only add ssl option when the URL explicitly requires it
...
app.post('/api/webhook/:key', async (req, res) => {
    ...
    const trunc = (s, n) => String(s || '').slice(0, n); // prevent oversized fields
           const lead = {
                 ...
                 phone:        trunc(b.phone, 30),
                 email:        trunc(b.email, 120),
                 source:       trunc(b.source, 60)   || 'Facebook Ads',
                 campaign:     trunc(b.campaign, 120),
                 adSet:        trunc(b.adSet, 120),
                 stage:        'new',
                 notes:        trunc(b.notes, 1000),
                 createdAt:    b.createdAt || todayISO(),
